...
instance VirtualIO StdIO where
   readInput (StdIO [] o) =  let l = unsafePerformIO $ getLine in readInput (StdIO (l ++ "\n") o)
   readInput (StdIO i o) = (fromEnum $ head i, StdIO (tail i) o)
   writeOutput 10 (StdIO i o) = unsafePerformIO $ do
     putStrLn o
     return (StdIO i [])
   writeOutput v (StdIO i o) = StdIO i (o ++ [toEnum v])
...
main = do
  ...
  let computer = boot{memory=program, io = StdIO [] []}
  putStrLn $ outCache $ io $ executeProgram computer
  putStrLn "Done";
