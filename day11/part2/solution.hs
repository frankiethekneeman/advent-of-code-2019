...
hullRobotIO = HullRobotIO Paint (0,0) (0,1) $ Map.singleton (0,0) 1
...
main = do
  ...
  let painted = Map.keys result
  let minX = minimum . map fst $ painted
  let maxX = maximum . map fst $ painted
  let minY = minimum . map snd $ painted
  let maxY = maximum . map snd $ painted
  let colors = [ [ Map.findWithDefault 0 (x,y) result | x <-[minX..maxX]] | y <- reverse [minY..maxY]]
  let text = unlines $ map (map colorToChar) colors
  putStrLn text
