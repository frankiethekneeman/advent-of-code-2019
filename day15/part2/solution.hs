...
rd computer = 
  let (value, remaining) = readInput $ io computer
    in (value, computer { io = remaining, state = if value == -1 then Halted else Running})
...
data OxygenIO = OxygenIO {
  position :: (Int, Int)
  , known :: Map.Map (Int, Int) Int
  , lastIns :: Int
}
initialize :: OxygenIO
initialize = OxygenIO (0,0) (Map.singleton (0,0) 1) 1
...
adjacent start = map (nextPosition start) [1..4]

bfs :: [[(Int, Int)]] -> Set.Set (Int, Int) -> Map.Map (Int, Int) Int -> Maybe (Int, Int)
bfs [] _ _ = Nothing
bfs (c:rest) seen lookup = case Map.lookup (head c) lookup of
  Nothing -> Just $ last c
  Just 0 -> bfs rest seen lookup
  _ -> let possible = adjacent $ head c
           notSeen = filter ((flip Set.notMember) seen) possible
           seen' = foldl (flip Set.insert) seen notSeen
           candidates = rest ++ map (:c) notSeen
         in bfs candidates seen' lookup

findNext :: (Int, Int) -> Map.Map (Int, Int) Int -> Maybe (Int, Int)
findNext start lookup =
  let dirs = adjacent start
      seen = Set.fromList dirs
      candidates = map (:[]) dirs
    in bfs candidates seen lookup

calcDir :: (Int, Int) -> (Int, Int) -> Int
calcDir (x1, y1) (x2, y2)
  | x1 == x2 && y2 == y1 + 1 = 1
  | x1 == x2 && y2 == y1 - 1 = 2
  | x1 == x2 + 1 && y2 == y1 = 3
...
foundToChar :: Int -> Char
foundToChar (-1) = ' '
foundToChar  0 = '#'
foundToChar  1 = '.'
foundToChar  2 = '@'

toStr :: (Int, Int) -> Map.Map (Int, Int) Int -> [Char]
toStr curr known
  | Map.size known == 0 = "?"
  | otherwise = 
    let ps = Map.keys known
        minX = minimum . map fst $ ps
        maxX = maximum . map fst $ ps
        minY = minimum . map snd $ ps
        maxY = maximum . map snd $ ps
        coords = [ [ (x, y) | x <- [minX..maxX] ] | y <- reverse [minY..maxY] ]
        toChar p = if p == curr then 'D' else foundToChar (Map.findWithDefault (-1) p known)
        txt = map (map toChar) coords
      in unlines txt 

instance VirtualIO OxygenIO where
  readInput o@OxygenIO{position=position, known=known} = case findNext position known of
    Nothing -> (-1, o { lastIns = -1 })
    Just next -> let dir = calcDir position next in (dir, o { lastIns = dir})
  writeOutput v (OxygenIO pos known lastIns) =
    let next = nextPosition pos lastIns
        known' = Map.insert next v known
        pos' = if v == 0 then pos else next
      in OxygenIO pos' known' lastIns

fill :: Int -> [(Int, Int)] -> Set.Set (Int, Int) -> Map.Map (Int, Int) Int -> Int
fill n [] _ _ = n - 1
fill n curr seen lookup = 
  let seen' = foldl (flip Set.insert) seen curr
      spread = concat $ map adjacent curr
      new = filter (flip Set.notMember seen') spread
      checker k = Map.findWithDefault 0 k lookup
      open = filter ((/=0).checker) new
    in fill (n + 1) open seen' lookup
--pathTo :: (Int, Int) -> [[(Int, Int)]] -> Set.Set (Int, Int) -> Map.Map (Int, Int) Int -> [(Int, Int)]
--pathTo target (c:rest) seen lookup
--  | target == head c = c
--  | otherwise =
--    let possible = adjacent $ head c
--        checker k = Map.findWithDefault 0 k lookup
--        open = filter ((/=0).checker) possible
--        notSeen = filter ((flip Set.notMember) seen) open
--        seen' = foldl (flip Set.insert) seen notSeen
--        candidates = rest ++ map (:c) notSeen
--      in pathTo target candidates seen' lookup
...
main = do
  ...
  let result = known $ io $ executeProgram boot { memory = program, io = initialize }
  let oxySystem = head $ Map.keys $ Map.filter (==2) result
  -- let path = pathTo oxySystem [[(0,0)]] (Set.singleton (0,0)) result
  print $ fill 0 [oxySystem] Set.empty result
