...
idle :: NicIO -> NicIO
idle n@NicIO{idleCount = idleCount} = n{idleCount = idleCount + 1}

instance VirtualIO NicIO where
  readInput io@NicIO{input = []} = (-1, idle io)
  readInput io@NicIO{input = i} = (head i,  io{input = (tail i), idleCount = 0})
  writeOutput d io@NicIO{dst = Nothing} = io{dst = Just d, idleCount = 0}
  writeOutput x io@NicIO{xCoord = Nothing} = io{xCoord = Just x, idleCount = 0}
  writeOutput y (NicIO i o (Just d) (Just x) _) = NicIO i (o ++ [Packet d x y]) Nothing Nothing 0

newNic :: Int -> NicIO
newNic address = NicIO [address] [] Nothing Nothing 0
...
isIdle :: Network -> Bool
isIdle network = minIdle > 1 && allQueuesEmpty
  where minIdle = minimum $ map (idleCount.io) $ elems network
        allQueuesEmpty = all null $ map (input.io) $ elems network

getAnswer :: Network -> Packet -> [Int]
getAnswer network natPacket 
  | idleP = (y natPacket):recurse
  | otherwise = recurse
  where (packets, awaitingDelivery) = step network
        ...
        delivered = foldl deliver awaitingDelivery deliverablePackets
        idleP = null packets && isIdle delivered
        network' = if idleP then deliver delivered natPacket else delivered
        natPacket' = case undeliverablePackets of 
          [] -> natPacket
          [(Packet 255 x y)] -> Packet 0 x y
        recurse = getAnswer network' natPacket'

getDuplicate :: Eq a => [a] -> a
getDuplicate (f:s:rest)
  | f == s = f
  | otherwise = getDuplicate (s:rest)
...
main = do
  ...
  let ys = getAnswer network (Packet (-1) 0 0)
  print $ getDuplicate ys
