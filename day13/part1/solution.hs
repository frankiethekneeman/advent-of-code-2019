...
reset :: Map.Map (Int, Int) Int -> JavanoidIO
reset s = JavanoidIO Nothing Nothing s
...
instance VirtualIO JavanoidIO where
    ...
    writeOutput value j = 
      let JavanoidIO { x = Just x, y = Just y, screen = screen } = j
        in reset $ Map.insert (x, y) value screen
...
main = do
  ...
  let result = screen $ io $ executeProgram boot { memory = program, io = reset Map.empty }
  print $ Map.size $ Map.filter (==2) result
