...
tpurn 1 h = let (dx, dy) = facing h in h { facing = (dy, -dx) }
...
data JavanoidIO = JavanoidIO {
  x :: Maybe Int
  , y :: Maybe Int
  , screen :: Map.Map (Int, Int) Int
  , score :: Int
}

reset :: Map.Map (Int, Int) Int -> Int -> JavanoidIO
reset screen score = JavanoidIO Nothing Nothing screen score

findTile :: Int -> JavanoidIO -> (Int, Int)
findTile tileType JavanoidIO{screen = screen} = 
  let tiles = Map.filter (==tileType) screen
    in if Map.size tiles > 1 
      then error "TOO MANY CANDIDATES"
      else (fst.head.Map.toList) tiles

ballPosition :: JavanoidIO -> Int
ballPosition = fst . findTile 4

paddlePosition :: JavanoidIO -> Int
paddlePosition = fst . findTile 3

instance VirtualIO JavanoidIO where
    writeOutput value j@JavanoidIO { x = Nothing } = j {x = Just value}
    writeOutput value j@JavanoidIO { y = Nothing } = j {y = Just value}
    writeOutput value j = 
      let JavanoidIO { x = Just x, y = Just y, screen = screen, score = score } = j
        in if (-1, 0) == (x, y)
          then reset screen value
          else reset (Map.insert (x, y) value screen) score
    readInput j = 
      let b = ballPosition j
          p = paddlePosition j
        in if b > p 
          then (1, j)
          else if b < p then (-1, j)
          else (0, j)
...
main = do
  ...
  let freeProgram = Map.insert 0 2 program
  let result = io $ executeProgram boot { memory = freeProgram, io = reset Map.empty 0 }
  print $ score result
