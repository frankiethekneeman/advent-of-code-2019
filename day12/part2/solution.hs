...
findLoop :: Int -> [Moon] -> [Int] -> (Moon -> Int) -> (Moon -> Int)-> Int
findLoop n moons target pFunc vFunc
  | target == partialState = n + 1
  | otherwise = findLoop (n + 1) nxt target pFunc vFunc
  where nxt = tick moons
        partialState = map pFunc nxt ++ map vFunc nxt

elcm :: Int -> Int -> Int
elcm a b = a * b `quot` (gcd a b)
...
main = do
  ...
  let xState = map x moons ++ map dx moons
  let yState = map y moons ++ map dy moons
  let zState = map z moons ++ map dz moons
  let xPeriod = findLoop 0 moons xState x dx
  let yPeriod = findLoop 0 moons yState y dy
  let zPeriod = findLoop 0 moons zState z dz
  print $ elcm xPeriod $ elcm yPeriod zPeriod
